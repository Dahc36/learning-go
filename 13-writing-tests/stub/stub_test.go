...
// This method doesn't scale well because you need to include every possible result or
// reimplement the struct for every test
...
// A better alternative is to create a stub struct that poxies method calls to function fields
...
// Then you can specify a function that returns the data that each method should return
func TestLogicGetPetNames2(t *testing.T) {
	...
	for _, d := range data {
		t.Run(d.name, func(t *testing.T) {
			// This way we can specify what each function will return in each case
